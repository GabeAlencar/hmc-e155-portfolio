/*
 * Module: display_mux
 * Author: Gabe Alencar gmenendezdealencar@g.hmc.edu
 * Date:   9/17/26
 * Parameters:
 *   MUX_W - width of the multiplexing counter; digits alternate every
 *           2**(MUX_W-1) clock cycles.
 */

module display_mux #(
    parameter int MUX_W = 17
) (
    input  logic       clk,
    input  logic       reset_b,
    input  logic [3:0] digit_left,
    input  logic [3:0] digit_right,
    output logic [6:0] seg,
    output logic [1:0] anode_n
);

    logic [MUX_W-1:0] mux_count;
    logic             digit_sel;
    logic [3:0]       nibble;

    // multiplexing counter
    counter #(.WIDTH(MUX_W), .MAX((2**MUX_W)-1)) u_mux_counter (.clk(clk), .reset_b(reset_b), .enable(1'b1), .count(mux_count));

    assign digit_sel = mux_count[MUX_W-1];
    assign nibble = digit_sel ? digit_right : digit_left;
    assign anode_n = digit_sel ? 2'b01 : 2'b10;
    sevenseg_decoder u_sevenseg (.s(nibble), .seg(seg));

endmodule