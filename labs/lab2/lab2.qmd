...
- **counter.sv** - a parameterized counter with synchronous enable and asynchronous
  reset. It counts to a settable MAX value and wraps to zero.
- **lab1_GA.sv** - the top-level module, which instantiates the HSOSC, decoder, counter, and contains the switch-to-LED assign statements
  along with the combinational logic for the LED from the counter's count value.
...
- **counter_tb.sv** - the counter is sequential and was tested for the following behavior: reset returns the count to zero, enable input starts and
  stops counting, and the counter wraps at MAX.
...
and wraps back to zero at the maximum count. The asynchronous reset is also
shown returning the count to zero mid-count.
