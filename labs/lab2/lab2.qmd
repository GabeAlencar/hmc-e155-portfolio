...
**Current-limiting resistors.** Figure 1 also shows the resistor sizing for
the segment and column LEDs, each targeting roughly 8 mA of LED current:
$R = (V_{CC} - V_{ON}) / I_{LED} = (3.3\text{V} - 2\text{V}) / 8\text{mA}
\approx 162.5\,\Omega$ for the column LEDs and $\approx 177.5\,\Omega$ for
the display segments, both rounded up to the nearest available 220 $\Omega$
resistor. The 2N3904 row-driver and 2N3906 anode/segment-driver transistors
use base resistors sized from $R = (V_{CC} - V_{BE})/I_B$, giving a
375 $\Omega$ minimum that was rounded up to a 10 k$\Omega$ base resistor for
current headroom, and confirming the 2N3906's collector current rating
comfortably exceeds the ~47.7 mA worst-case load of driving all seven
segments at once ($I_{max} = 7 \times I_{seg} \ll I_C$).

### Block Diagram

![Figure 2: `lab2_GA` top-level block diagram](images/lab2BlockDiagram.jpg)

Figure 2 shows the top-level module's structure: the `HSOSC` primitive
clocks the `mux_counter`, whose MSB (`digit_sel`) selects between `s1` and
inverted `s2` at the shared `sevenseg_decoder`'s input and drives the
`anode_n` outputs, while the free-running `scanner` instance drives the
`rows` outputs independently and `cols` is inverted directly onto `col_led`.

### Schematic

![Figure 3: full board-level schematic](images/lab2Schematic.jpg)

Figure 3 shows the complete board-level schematic: the `s1`/`s2` DIP switch
banks (with pull-up resistors on `s2`) feeding the FPGA directly, the 4x4
keypad matrix with pull-up resistors on the columns and 2N3904 NPN
transistors driving each row, the 2N3906 PNP transistors switching the
common-anode digit supply, and 220 $\Omega$ current-limiting resistors on
both the seven-segment LEDs and the column-indicator LEDs.
...
- **Hardware:** the shared-decoder multiplexing scheme, 366 Hz digit refresh
  ...
  in simulation, and the schematic (Figure 3) shows the current-limiting
  resistor values and pull-up wiring for the keypad columns needed to read
